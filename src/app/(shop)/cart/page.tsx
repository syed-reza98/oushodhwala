    ...
    return (
      <div className="pt-16 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {t("পছন্দের ঔষধ ও পণ্য যোগ করুন।", "Add your favorite medicines and products.")}
        </p>
        <Link
  ...
  return (
    <div className="pt-4">
      <h1 className="text-base font-bold">
        {t(`আপনার কার্ট (${t.n(cart.length)} আইটেম)`, `Your cart (${t.n(cart.length)} items)`)}
      </h1>
        ...
        <div className="space-y-2">
          {cart.map((l) => (
            <div key={l.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {stockOf(l.id, l.kind) !== null && l.qty >= (stockOf(l.id, l.kind) as number) && (
                  <p className="text-[10px] font-semibold text-sale">
                    {t(
                      `স্টকে আছে মাত্র ${t.n(stockOf(l.id, l.kind) as number)} টি`,
                      `Only ${t.n(stockOf(l.id, l.kind) as number)} left in stock`,
                    )}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2 rounded-lg border border-border px-2 py-1">
                <button onClick={() => setQty(l.id, l.qty - 1)} aria-label={t("কমান", "Decrease")}>
              ...
              <button onClick={() => remove(l.id)} aria-label={t("মুছুন", "Remove")}>
        ...
        <aside className="h-fit rounded-xl border border-border bg-card p-4">
          ...
          {applied && (
            <p className="mt-1 text-[11px] font-semibold text-primary">
              {t(
                `${applied.code} প্রয়োগ হয়েছে (${t.n(applied.discountPct)}% ছাড়)`,
                `${applied.code} applied (${t.n(applied.discountPct)}% off)`,
              )}
            </p>
          )}
          {invalid && !applied && (
            <p className="mt-1 text-[11px] text-destructive">
              {t(
                "কুপন কোডটি সঠিক নয় বা সর্বনিম্ন অর্ডার পূরণ হয়নি",
                "Coupon code is invalid or minimum order not met",
              )}
            </p>
          )}
          ...
          <dl className="mt-3 space-y-1.5 text-xs">
            ...
            <Row
              ...
              v={delivery === 0 ? t("ফ্রি", "Free") : t.money(delivery)}
            />
          ...
          <p className="mt-2 text-center text-[10px] text-muted-foreground">
            {t(
              `${t.money(settings.freeDeliveryMin)}+ অর্ডারে ফ্রি ডেলিভারি`,
              `Free delivery on orders over ${t.money(settings.freeDeliveryMin)}`,
            )}
          </p>
        </aside>
      </div>
