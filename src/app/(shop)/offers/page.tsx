    ...
    <div className="pt-4">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        {offers.map((c) => (
          <div
            key={c.id}
            className="flex items-center gap-3 rounded-xl border border-dashed border-primary bg-secondary p-3"
          >
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {deals.map(({ p, off }) => (
          <div key={p.id}>
            <ProductCard p={p} />
            <p className="mt-1 text-center text-[10px] font-semibold text-sale">
              {t(
                `সাশ্রয় ${t.money(p.mrp - p.price)} (${t.n(Math.round(off * 100))}%)`,
                `Save ${t.money(p.mrp - p.price)} (${t.n(Math.round(off * 100))}%)`,
              )}
            </p>
          </div>
        ))}
