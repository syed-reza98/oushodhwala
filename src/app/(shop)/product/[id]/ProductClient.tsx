...
function normalizePayload(raw: {
  ...
  return {
    product: mapProduct(raw.row),
    related: (raw.related ?? []).map(mapProduct),
    variants: (raw.variants ?? []) as Variant[],
    generic: raw.generic ?? null,
  };
}
...
export default function ProductClient({
  ...
  useEffect(() => {
    setData(normalizePayload(initial));
  }, [initial]);
    ...
    <div className={reading ? "mx-auto max-w-2xl pt-4" : "pt-4"}>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                className={`h-3.5 w-3.5 ${i < Math.round(p.rating) ? "fill-current text-sale" : "text-muted-foreground"}`}
              />
            ))}
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            {p.mrp > p.price && (
              <span className="text-sm text-muted-foreground line-through">৳{num(p.mrp)}</span>
            )}
          </div>

          <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <div>
              <dt className="text-muted-foreground">{isEn ? "Brand" : "ব্র্যান্ড"}</dt>
              <dd className="font-semibold">{p.brand}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">{isEn ? "Generic" : "জেনেরিক"}</dt>
              <dd className="font-semibold">{p.generic}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">{isEn ? "Dosage form" : "ধরন"}</dt>
              <dd className="font-semibold">{p.form}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">{isEn ? "Pack size" : "প্যাক সাইজ"}</dt>
              <dd className="font-semibold">{p.pack}</dd>
            </div>
            {p.strength && (
              <div>
                <dt className="text-muted-foreground">{isEn ? "Strength" : "মাত্রা"}</dt>
                <dd className="font-semibold">{p.strength}</dd>
              </div>
            )}
            {pick(
              lang,
              p.therapeuticClass || g("therapeutic_class"),
              p.therapeuticClassEn || g("therapeutic_class_en"),
            ) && (
              <div>
                <dt className="text-muted-foreground">
                  {isEn ? "Therapeutic class" : "থেরাপিউটিক ক্লাস"}
                </dt>
          ...
          {variants.length > 1 && (
            <div className="mt-3">
              <p className="text-xs font-bold">
                {isEn ? "Available strengths & forms" : "উপলব্ধ মাত্রা ও ধরন"}
              </p>
          ...
          {p.rx && (
            <p className="mt-3 rounded-lg border border-sale/40 bg-sale/10 p-2 text-[11px] font-semibold text-sale">
              {isEn
                ? "A doctor's prescription is required to buy this medicine."
                : "এই ঔষধটি কিনতে ডাক্তারের প্রেসক্রিপশন প্রয়োজন।"}{" "}
              <Link href="/prescription" className="underline">
                {isEn ? "Upload" : "আপলোড করুন"}
              </Link>
            </p>
          )}
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            {line ? (
              <div className="flex flex-1 items-center gap-2">
                ...
                <Link
                  href="/cart"
                  className="ml-auto rounded-lg bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground"
                >
                  কার্ট দেখুন
                </Link>
                <button
                  ...
                  className="rounded-lg border border-border px-3 py-2.5 text-xs font-semibold"
                >
                  সরান
                </button>
              </div>
          ...
          <div className="mt-3">
            <ShareButton
              title={isEn ? p.en || p.name : p.name}
              text={`${isEn ? p.en || p.name : p.name} — Oushodhwala`}
            />
          </div>
      ...
      {(() => {
        ...
        const raw: Raw[] = (
          [
            {
              t: isEn ? "Product Description" : "পণ্যের বিবরণ",
              body: cleanMedText(pick(lang, p.desc, p.descEn)),
            },
            {
              t: isEn ? "Indications" : "নির্দেশনা",
              body: cleanMedText(
                pick(lang, p.indications || g("indications"), p.indicationsEn || g("indications_en")),
              ),
            },
            {
              t: isEn ? "Pharmacology" : "ফার্মাকোলজি",
              body: cleanMedText(pick(lang, g("pharmacology"), g("pharmacology_en"))),
            },
            {
              t: isEn ? "Dosage & Administration" : "মাত্রা ও সেবনবিধি",
              kind: "dosage",
              body: cleanMedText(pick(lang, p.dosage || g("dosage"), p.dosageEn || g("dosage_en"))),
            },
            {
              t: isEn ? "Interaction" : "ঔষধের মিথস্ক্রিয়া",
              body: cleanMedText(pick(lang, g("interaction"), g("interaction_en"))),
            },
            {
              t: isEn ? "Contraindications" : "প্রতিনির্দেশনা",
              kind: "warning",
              body: cleanMedText(
                pick(
                  lang,
                  p.contraindications || g("contraindications"),
                  p.contraindicationsEn || g("contraindications_en"),
                ),
              ),
            },
            {
              t: isEn ? "Side Effects" : "পার্শ্বপ্রতিক্রিয়া",
              kind: "side-effects",
              body: cleanMedText(
                pick(lang, p.sideEffects || g("side_effects"), p.sideEffectsEn || g("side_effects_en")),
              ),
            },
            {
              t: isEn ? "Pregnancy & Lactation" : "গর্ভাবস্থায় ও স্তন্যদানকালে",
              kind: "pregnancy",
              body: cleanMedText(
                pick(lang, p.pregnancy || g("pregnancy"), p.pregnancyEn || g("pregnancy_en")),
              ),
            },
            {
              t: isEn ? "Precautions & Warnings" : "সতর্কতা",
              kind: "warning",
              body: cleanMedText(
                pick(lang, p.precautions || g("precautions"), p.precautionsEn || g("precautions_en")),
              ),
            },
            {
              t: isEn ? "Use in Special Populations" : "বিশেষ ক্ষেত্রে ব্যবহার",
              body: cleanMedText(pick(lang, g("special_populations"), g("special_populations_en"))),
            },
            {
              t: isEn ? "Overdose Effects" : "মাত্রাধিক্য",
              kind: "warning",
              body: cleanMedText(pick(lang, g("overdose"), g("overdose_en"))),
            },
            {
              t: isEn ? "Therapeutic Class" : "থেরাপিউটিক ক্লাস",
              body: cleanMedText(
                pick(
                  lang,
                  p.therapeuticClass || g("therapeutic_class"),
            ...
            {
              t: isEn ? "Storage Conditions" : "সংরক্ষণ",
              body: cleanMedText(pick(lang, p.storage || g("storage"), p.storageEn || g("storage_en"))),
            },
            ...(p.manufacturer
              ? [{ t: isEn ? "Manufacturer" : "প্রস্তুতকারক", body: p.manufacturer }]
              : []),
          ] as Raw[]
        ).filter((s) => Boolean(s.body));

        const kinds = new Map(raw.map((s) => [s.t, s.kind]));
        const sections: MedSection[] = dedupeSections(raw.map((s) => ({ t: s.t, body: s.body }))).map(
          (s) => ({
            title: s.t,
            body: s.body,
        ...
        return (
          <div className="pt-6">
            ...
            <MedSections
              sections={sections}
              reading={reading}
              badgeText={isEn ? "Important" : "গুরুত্বপূর্ণ"}
            />
          </div>
        );
      })()}
