    ...
    <div className="pt-4">
      ...
      <p className="text-xs text-muted-foreground">
        {t(
          ...
          "Talk directly with verified doctors via phone, WhatsApp or video call",
        )}
      </p>
      ...
      <p className="mt-3 rounded-lg bg-secondary p-3 text-[11px] font-semibold text-primary-dark">
        ...
        {t(
          ...
          "Book an appointment by choosing a date and time before the call — after payment you get an invoice, chat and saved recording.",
        )}{" "}
      ...
      {list.length === 0 && (
        <p className="mt-4 text-xs text-muted-foreground">{t("এই মুহূর্তে কোনো ডাক্তার নেই।", "No doctors available right now.")}</p>
      )}
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((d) => {
          ...
          const waText = encodeURIComponent(
            t(
              `আসসালামু আলাইকুম, আমি ঔষধওয়ালা থেকে ${d.name} এর সাথে পরামর্শ নিতে চাই।`,
              `Hello, I would like to consult with ${d.name} via Oushodhwala.`,
            ),
          );
          return (
            <article key={d.id} className="rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
              ...
              <div className="mt-3 flex items-center">
                ...
                <Link
                  href={`/book-doctor/${encodeURIComponent(d.id)}`}
