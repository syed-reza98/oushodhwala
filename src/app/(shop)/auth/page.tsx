...
import { loginUser, registerUser, requestPasswordReset } from "@/server/actions/auth";
...
export default function AuthPage() {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "forgot") {
        ...
        if (res.token) {
          // Local/dev only — paste into /reset-password?token=…
          console.info("Password reset token:", res.token);
          toast.message(t("ডেভ: টোকেন কনসোলে", "Dev: token in console"));
        }
        return;
      }
      if (mode === "signup") {
      ...
      const friendly = /invalid|credentials/i.test(msg)
        ? t("ইমেইল বা পাসওয়ার্ড সঠিক নয়", "Incorrect email or password")
        : /already registered|Email already/i.test(msg)
  ...
  const perks = [
    ...
    { icon: Clock, t: t("জরুরি ডেলিভারি ৩০–৬০ মিনিট", "Express delivery in 30–60 min") },
    {
      icon: CheckCircle2,
      t: t(
        "অর্ডার, প্রেসক্রিপশন ও লয়ালটি পয়েন্ট এক জায়গায়",
        "Orders, prescriptions & loyalty in one place",
      ),
    },
  ];

  const field =
    "w-full rounded-xl border border-border bg-background px-10 py-3 text-base outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/20";
      ...
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-3xl border border-border bg-card shadow-[var(--shadow-elevated)] lg:grid-cols-2">
        <aside className="hidden flex-col justify-between bg-navy p-8 text-navy-foreground lg:flex">
          ...
          <p className="mt-8 text-[11px] opacity-60">
            {t(
              "ঔষধওয়ালা — Shondhaan (Yess Bangla Private Limited এর সিস্টার কনসার্ন)",
              "Oushodhwala — part of Shondhaan, a sister concern of Yess Bangla Private Limited",
            )}
          </p>
        </aside>
        ...
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {mode === "forgot"
              ...
              : t(
                  "ইমেইল ও পাসওয়ার্ড দিয়ে ঔষধওয়ালা একাউন্ট ব্যবহার করুন।",
                  "Use your email and password to access your Oushodhwala account.",
                )}
          </p>
          ...
          {sent ? (
            <div className="mt-5 rounded-2xl border border-primary/30 bg-primary/5 p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                {sent === "reset"
                  ? t(
                      "ঠিকানায় একটি রিসেট লিংক পাঠিয়েছি। ইনবক্স/স্প্যাম চেক করুন।",
                      "with a reset link. Check your inbox or spam folder.",
                    )
                  : t(
                      "ঠিকানায় একটি কনফার্মেশন লিংক পাঠিয়েছি। লিংকে ক্লিক করলেই একাউন্ট চালু হবে।",
                  ...
                  <div className="relative">
                    ...
                    {form.phone && !phoneOk && (
                      <p className="mt-1 text-[11px] text-sale">
                        {t("১১ ডিজিটের সঠিক নম্বর দিন", "Enter a valid 11-digit number")}
                      </p>
                    )}
                  </div>
                </>
