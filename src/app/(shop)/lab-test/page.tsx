  ...
  return (
    <div className="pt-4">
      ...
      <p className="text-xs text-muted-foreground">
        {t("ঘরে বসে স্যাম্পল সংগ্রহ · ২৪ ঘণ্টায় ডিজিটাল রিপোর্ট", "Home sample collection · Digital report in 24 hours")}
      </p>
      ...
      {list.length === 0 && (
        <p className="mt-4 text-xs text-muted-foreground">{t("কোনো টেস্ট পাওয়া যায়নি।", "No tests found.")}</p>
      )}
          ...
          return (
            <article key={test.id} className="rounded-xl border border-border bg-card p-3">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                {test.mrp > test.price && (
                  <span className="text-[10px] text-muted-foreground line-through">{t.money(test.mrp)}</span>
                )}
                <button
                  disabled={inCart}
        ...
        {done ? (
          <p className="mt-2 rounded-lg bg-secondary p-3 text-xs font-semibold text-primary-dark">
            ...
            {t(
              ...
              "Booking received. Our phlebotomist will reach your address at the scheduled time.",
            )}
          </p>
        ) : (
