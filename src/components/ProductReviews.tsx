        ...
        {reviews.length > 0 && (
          <>
            ...
            <span className="text-xs text-muted-foreground">
              {t.n(Number(avg.toFixed(1)))} · {t.n(reviews.length)} {t("রিভিউ", "reviews")}
            </span>
          </>
        )}
      </div>
        ...
        {!user ? (
          ...
          <>
            <p className="text-xs font-semibold">
              {mine
                ? t("আপনার রিভিউ সম্পাদনা করুন", "Edit your review")
                : t("আপনার মতামত দিন", "Share your experience")}
            </p>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              >
                {save.isPending
                  ? t("পাঠানো হচ্ছে...", "Saving...")
                  : t("রিভিউ জমা দিন", "Submit review")}
              </button>
              {done && (
                <span className="text-[11px] font-semibold text-primary">
                  {t("ধন্যবাদ! রিভিউ যুক্ত হয়েছে।", "Thanks! Review saved.")}
                </span>
              )}
              {save.isError && (
                <span className="text-[11px] text-sale">
                  {t("সমস্যা হয়েছে, আবার চেষ্টা করুন।", "Something went wrong.")}
                </span>
              )}
            </div>
          </>
          ...
          <article key={r.id} className="rounded-xl border border-border bg-card p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold">
                {r.author_name || t("গ্রাহক", "Customer")}
              </span>
              {r.verified && (
